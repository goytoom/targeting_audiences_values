...
# Import ------------------------------------------------------------------
  ...
  dr <- read_csv("../data/raw/qualtrics_export_order.csv")[-1:-2,]
...
# Prepare -----------------------------------------------------------------
  # Select variables
  dw <- dr %>% 
    select(prolific_pid, exclude, strata, contains("q"), starts_with("s_"), -contains(c("first_click", "last_click")), -ends_with("_DO"), fl_19_do)
  ...
  dw2 <- dw2 %>% distinct(prolific_pid, .keep_all = TRUE)
  
  #take fl_19_do column -> take first number from string (first question shown) -> minus 3 for qualtrics number
  #create column in final data set
  #run additional model that only includes the first question (or includes order as variable)
    #filter out all the responses that arent first -> see if effects hold
...
# Export ------------------------------------------------------------------
  ...
  # Export as .rds
  write_rds(dl, "../data/dl_order.rds")
  
  # Export as .csv
  write_excel_csv(dl, "../data/csv/dl_order.csv")
