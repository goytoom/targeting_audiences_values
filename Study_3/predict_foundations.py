...
predict moral framing of tweets
takes hydrated tweets from data/tweets/* as input
saves classified tweets in 
@author: suhai
"""
    ...
    for mode, threshold in zip(modes, thresholds):
        ...
        for data_file in df_list:
            ...
            if "tweets" in data_file or "stance" in data_file:
                df_dropped.to_csv("results/" + data_file.split("/")[-1].split(".")[0].split("_")[0] + "_" + mode + ".csv", index=False)
            else:
