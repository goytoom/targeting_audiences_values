...
Load annotated training corpus (mftc_cleaned.csv)
Convert annotations according to mode (moral vs nonmoral, individualizing vs binding, all foundations)
Tokenize and create embaddings from texts
Save as mftc_cleaned_combined_MODE
...
if __name__ == '__main__':
    ...
    # mode = "binding"
    for mode in ["binding", "moral"]:
        if not os.path.isfile("data/mftc_cleaned_combined_" + mode + ".csv"):
            transformData(mode)
        else:
            pass
        
        data_file="data/mftc_cleaned_combined_" + mode + ".csv"
        if not (os.path.file("data/train_" + mode + ".pkl") & os.path.isfile("data/test_" + mode + ".pkl")):
